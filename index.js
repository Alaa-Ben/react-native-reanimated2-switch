...
const Switch = ({
  ...
  const TRACK_CIRCLE_WIDTH =
    (containerStyle?.width || SWITCH_CONTAINER_WIDTH) -
    circleWidth -
  ...
  const onPress = ({ nativeEvent: { state } }) => {
    ...
    translateX.value = withSpring(
      isToggled ? 0 : TRACK_CIRCLE_WIDTH,
      animationConfig
    );
  };
  ...
  return (
    ...
    >
      <Animated.View
        style={[animatedContainerStyle, styles.switchContainer, containerStyle]}
      >
          ...
          <Animated.View
            style={[
              ...
              styles.circle,
              {
                ...
                width: containerStyle?.width / 2 || CIRCLE_WIDTH,
                height: containerStyle?.height || CIRCLE_WIDTH,
              },
              circleStyle,
            ]}
          >
